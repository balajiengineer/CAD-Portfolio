<div align="center">

![Project](https://img.shields.io/badge/Project-Completed-success?style=for-the-badge)
![Product](https://img.shields.io/badge/Product%20Design-74ACDF?style=for-the-badge)
![Injection](https://img.shields.io/badge/Injection%20Moulding-0A66C2?style=for-the-badge)
![CAD](https://img.shields.io/badge/Creo-74ACDF?style=for-the-badge)

# 📡 RFID Hybrid Enclosure

### Mechanical Design | Product Development | Injection Moulding | CNC | GD&T

![Status](https://img.shields.io/badge/Project-Completed-success)
![CAD](https://img.shields.io/badge/CAD-Creo-blue)
![Manufacturing](https://img.shields.io/badge/Manufacturing-DFM-orange)

---
## Mechanical Design Portfolio

### Designed by

## Balaji Jegan Maheshwaran T

Mechanical Design Engineer

</div>

---

# 📌 Project Overview

The RFID Hybrid Enclosure was designed as a robust industrial housing for an RFID-based electronic system. The design integrates plastic, aluminium and composite components into a compact enclosure while ensuring structural integrity, manufacturability and ease of assembly.

The project covered the complete mechanical design process from concept development through production-ready CAD models, engineering drawings and manufacturing documentation.

---

# 🎯 Project Objectives

- Develop a compact RFID enclosure
- Improve structural rigidity
- Ensure manufacturability
- Simplify assembly
- Protect internal electronic components
- Reduce manufacturing complexity

---

# 🛠 Scope of Work

- Complete 3D CAD Modelling
- Plastic Enclosure Design
- Aluminium Support Plate Design
- CFRP PCB Support Plate
- Mounting Clamp Design
- Engineering Drawings
- Exploded Assembly
- BOM Preparation
- Mass Property Validation
- Tolerance Analysis

---

# 🏭 Manufacturing Processes

The enclosure was designed considering multiple manufacturing processes:

- Plastic Injection Moulding
- CNC Machining
- CNC Routing
- Waterjet Cutting
- Sheet Metal Bending

---

# ⚙ Key Design Considerations

- Uniform Wall Thickness
- 2° Draft Angle
- Minimum Bend Radius
- Standard Fasteners (SS304)
- Manufacturing Tolerances (±0.2 mm)
- Deburring & Surface Finish
- Assembly Accessibility

---

# 📐 Engineering Activities

- Product Design
- Plastic Part Design
- Injection Mould Design Considerations
- Sheet Metal Design
- CNC Component Design
- Assembly Modelling
- Engineering Drawings
- GD&T
- BOM Creation
- DFM
- DFA

---

# 💻 Software Used

- Creo Parametric
- AutoCAD

---

# 🖼️ Project Gallery

## Complete Assembly

<img src="Images/Assembly.png" width="900"/>

---

## Exploded Assembly

<img src="Images/Exploded_View.png" width="900"/>

---

# 📄 Complete Project Portfolio

The complete project portfolio includes:

- Project Summary
- CAD Design
- Manufacturing Process Selection
- Engineering Drawings
- GD&T
- Assembly Documentation
- BOM
- Mass Property Validation
- Design Considerations

📥 **[View Complete Portfolio](./RFID_Hybrid_Enclosure_Portfolio.pdf)**

---

# 🏆 Skills Demonstrated

- Product Design
- Mechanical Design
- Plastic Design
- Injection Moulding Design
- Sheet Metal Design
- CNC Design
- GD&T
- Assembly Design
- Engineering Documentation
- Design for Manufacturing
- Design for Assembly

---

<div align="center">

⭐ Mechanical Product Design Portfolio

Designed by **Balaji Jegan Maheshwaran T**

</div>
